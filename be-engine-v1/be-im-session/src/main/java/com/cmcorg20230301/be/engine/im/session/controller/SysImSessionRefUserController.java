...
public class SysImSessionRefUserController {
    ...
    public ApiResultVO<String> joinUserIdSet(
        @RequestBody @Valid SysImSessionRefUserJoinUserIdSetDTO dto) {
        return ApiResultVO.okMsg(baseService.joinUserIdSet(dto));
    }

    @Operation(summary = "查询：当前会话的用户信息，map")
    @PostMapping("/query/refUserInfoMap")
    public ApiResultVO<LongObjectMapVO<SysImSessionRefUserQueryRefUserInfoMapVO>> queryRefUserInfoMap(
        @RequestBody @Valid NotNullIdAndLongSet notNullIdAndLongSet) {
