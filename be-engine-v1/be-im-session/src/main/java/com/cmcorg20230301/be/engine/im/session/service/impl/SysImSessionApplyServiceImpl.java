...
public class SysImSessionApplyServiceImpl extends ServiceImpl<SysImSessionApplyMapper, SysImSessionApplyDO>
     ...
     */
    @Override
    public Page<SysImSessionApplyPrivateChatFindNewPageVO>
     ...
     */
    @Override
    public Page<SysImSessionApplyPrivateChatApplyInitiateSelfPageVO>
        ...
        Page<SysImSessionApplyDO> pageTemp =
            lambdaQuery().eq(BaseEntityNoIdSuper::getTenantId, tenantId).eq(SysImSessionApplyDO::getUserId, userId)
                .eq(SysImSessionApplyDO::getShowFlag, true).select(SysImSessionApplyDO::getPrivateChatApplyTargetUserId)
        ...
        Set<Long> applyUserIdSet = pageTemp.getRecords().stream()
            .map(SysImSessionApplyDO::getPrivateChatApplyTargetUserId).collect(Collectors.toSet());
        ...
        List<SysImSessionApplyPrivateChatApplyInitiateSelfPageVO> list = new ArrayList<>(sysUserInfoDOList.size());

        for (SysUserInfoDO item : sysUserInfoDOList) {

            SysImSessionApplyPrivateChatApplyInitiateSelfPageVO sysImSessionApplyPrivateChatApplyInitiateSelfPageVO =
                new SysImSessionApplyPrivateChatApplyInitiateSelfPageVO();

            sysImSessionApplyPrivateChatApplyInitiateSelfPageVO.setUserId(item.getId());
            sysImSessionApplyPrivateChatApplyInitiateSelfPageVO.setNickname(item.getNickname());

            sysImSessionApplyPrivateChatApplyInitiateSelfPageVO.setAvatarUrl(avatarUrlMap.get(item.getAvatarFileId()));

            list.add(sysImSessionApplyPrivateChatApplyInitiateSelfPageVO);

        }

        return new Page<SysImSessionApplyPrivateChatApplyInitiateSelfPageVO>().setTotal(pageTemp.getTotal())
            .setRecords(list);
     ...
     */
    @Override
    public Page<SysImSessionApplyPrivateChatApplySelfPageVO>
        ...
        Page<SysImSessionApplyDO> pageTemp = lambdaQuery().eq(BaseEntityNoIdSuper::getTenantId, tenantId)
            .eq(SysImSessionApplyDO::getPrivateChatApplyTargetUserId, userId).eq(SysImSessionApplyDO::getShowFlag, true)
            .select(SysImSessionApplyDO::getUserId).page(dto.updateTimeDescDefaultOrderPage(true));
        ...
        Set<Long> applyUserIdSet =
            pageTemp.getRecords().stream().map(SysImSessionApplyDO::getUserId).collect(Collectors.toSet());
        ...
        for (SysUserInfoDO item : sysUserInfoDOList) {
            ...
            sysImSessionApplyPrivateChatApplySelfPageVO.setAvatarUrl(avatarUrlMap.get(item.getAvatarFileId()));
     ...
     */
    @Override
    public Page<SysImSessionApplyPrivateChatSelfPageVO>
        privateChatPageSelf(SysImSessionApplyPrivateChatSelfPageDTO dto) {
    ...
    public String privateChatApply(SysImSessionApplyPrivateChatApplyDTO dto) {
        ...
        return RedissonUtil.doLock(key, () -> {
            ...
            SysImSessionApplyDO sysImSessionApplyDO =
                lambdaQuery().eq(SysImSessionApplyDO::getUserId, userId).eq(BaseEntityNoIdSuper::getTenantId, tenantId)
    ...
    public static String getPrivateChatApplyKey(Long userId, Long targetUserId) {
        ...
        return BaseRedisKeyEnum.PRE_SYS_IM_SESSION_APPLY_PRIVATE_CHAT.name() + Math.min(userId, targetUserId)
    ...
    public String privateChatAgree(NotEmptyIdSet notEmptyIdSet) {
        ...
        Long count = lambdaQuery().eq(SysImSessionApplyDO::getPrivateChatApplyTargetUserId, userId)
            .eq(BaseEntityNoIdSuper::getTenantId, tenantId).in(SysImSessionApplyDO::getUserId, applyUserIdSet).count();
    ...
    private String handlePrivateChatAgree(Long userId, Long tenantId, Set<Long> applyUserIdSet, Date date) {
        ...
        List<SysImSessionApplyDO> sysImSessionApplyDOList =
            lambdaQuery().eq(SysImSessionApplyDO::getPrivateChatApplyTargetUserId, userId)
                .eq(BaseEntityNoIdSuper::getTenantId, tenantId).in(SysImSessionApplyDO::getUserId, applyUserIdSet)
                .eq(SysImSessionApplyDO::getStatus, SysImSessionApplyStatusEnum.APPLYING)
                .select(SysImSessionApplyDO::getId, SysImSessionApplyDO::getSessionId, SysImSessionApplyDO::getUserId)
                .list();
        ...
        if (CollUtil.isNotEmpty(enabelSessionIdSet)) {
            ...
            // 批量：让会话关联的用户恢复可用状态
            sysImSessionRefUserService.lambdaUpdate().in(SysImSessionRefUserDO::getSessionId, enabelSessionIdSet)
                .set(SysImSessionRefUserDO::getShowFlag, true).set(SysImSessionRefUserDO::getEnableFlag, true)
                .set(SysImSessionRefUserDO::getBlockFlag, false).set(BaseEntityNoIdSuper::getUpdateTime, date).update();
    ...
    public String privateChatReject(SysImSessionApplyPrivateChatRejectDTO dto) {
        ...
        return RedissonUtil.doLock(key, () -> {
            ...
            boolean update = lambdaUpdate().eq(SysImSessionApplyDO::getPrivateChatApplyTargetUserId, userId)
                .eq(BaseEntityNoIdSuper::getTenantId, tenantId).eq(SysImSessionApplyDO::getUserId, applyUserId)
                .eq(SysImSessionApplyDO::getStatus, SysImSessionApplyStatusEnum.APPLYING)
                .set(SysImSessionApplyDO::getShowFlag, true)
    ...
    public String privateChatBlock(NotNullId notNullId) {
        ...
        return RedissonUtil.doLock(key, () -> {
            ...
            SysImSessionApplyDO sysImSessionApplyDO = lambdaQuery()
                .eq(SysImSessionApplyDO::getPrivateChatApplyTargetUserId, userId)
                .eq(BaseEntityNoIdSuper::getTenantId, tenantId).eq(SysImSessionApplyDO::getUserId, applyUserId)
                .select(SysImSessionApplyDO::getId, SysImSessionApplyDO::getStatus, SysImSessionApplyDO::getSessionId)
                .one();
            ...
            lambdaUpdate().eq(SysImSessionApplyDO::getId, sysImSessionApplyDO.getId())
                ...
                .set(SysImSessionApplyDO::getBlockPreStatus, sysImSessionApplyDO.getStatus()).update();

            sysImSessionRefUserService.lambdaUpdate()
                .eq(SysImSessionRefUserDO::getSessionId, sysImSessionApplyDO.getSessionId())
                .set(SysImSessionRefUserDO::getBlockFlag, true).set(BaseEntityNoIdSuper::getUpdateTime, date).update();
    ...
    public String privateChatBlockCancel(NotEmptyIdSet notEmptyIdSet) {
        ...
        return RedissonUtil.doMultiLock("", ketSet, () -> {
            ...
            List<SysImSessionApplyDO> sysImSessionApplyDOList =
                lambdaQuery().eq(SysImSessionApplyDO::getPrivateChatApplyTargetUserId, userId)
                    ...
                    .select(SysImSessionApplyDO::getId, SysImSessionApplyDO::getSessionId,
                        SysImSessionApplyDO::getBlockPreStatus)
                    .list();
            ...
            sysImSessionRefUserService.lambdaUpdate().in(SysImSessionRefUserDO::getSessionId, sessionIdSet)
                .set(SysImSessionRefUserDO::getBlockFlag, false).set(BaseEntityNoIdSuper::getUpdateTime, date).update();
    ...
    public String privateChatApplyCancel(NotNullId notNullId) {
        ...
        return RedissonUtil.doLock(key, () -> {
            ...
            boolean remove =
                lambdaUpdate().eq(SysImSessionApplyDO::getPrivateChatApplyTargetUserId, privateChatApplyTargetUserId)
                    .eq(BaseEntityNoIdSuper::getTenantId, tenantId).eq(SysImSessionApplyDO::getUserId, userId)
    ...
    public String privateChatDelete(NotNullId notNullId) {
        ...
        boolean update = sysImSessionRefUserService.lambdaUpdate()
            .and(i -> i.eq(SysImSessionRefUserDO::getUserId, userId).or()
                .eq(SysImSessionRefUserDO::getPrivateChatRefUserId, userId))
            .eq(BaseEntityNoIdSuper::getTenantId, tenantId).eq(SysImSessionRefUserDO::getSessionId, sessionId)
            .set(SysImSessionRefUserDO::getEnableFlag, false).set(BaseEntityNoIdSuper::getUpdateTime, date).update();
