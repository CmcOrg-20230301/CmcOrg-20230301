    ...
    public UserSelfInfoVO userSelfInfo() {
        ...
        MyThreadUtil.execute(() -> {
            ...
            SysUserDO sysUserDO = lambdaQuery().eq(BaseEntity::getId, currentUserId)
                .select(SysUserDO::getEmail, SysUserDO::getPassword, SysUserDO::getSignInName, SysUserDO::getPhone,
                    SysUserDO::getWxOpenId, BaseEntity::getCreateTime, SysUserDO::getWxAppId)
                .one();
            ...
            if (sysUserDO != null) {
                ...
                sysUserSelfInfoVO
                    .setWxOpenId(StrUtil.hide(sysUserDO.getWxOpenId(), 3, sysUserDO.getWxOpenId().length() - 4)); // 脱敏：只显示前
                                                                                                                  // 3位，后
                                                                                                                  // 4位
                sysUserSelfInfoVO
                    .setWxAppId(StrUtil.hide(sysUserDO.getWxAppId(), 3, sysUserDO.getWxAppId().length() - 4)); // 脱敏：只显示前
                                                                                                               // 3位，后
                                                                                                               // 4位
        ...
        MyThreadUtil.execute(() -> {
            ...
            SysUserSingleSignInDO sysUserSingleSignInDO = ChainWrappers.lambdaQueryChain(sysUserSingleSignInMapper)
                .eq(SysUserSingleSignInDO::getId, currentUserId)
                ...
                .select(SysUserSingleSignInDO::getWxOpenId, SysUserSingleSignInDO::getPhone,
                    SysUserSingleSignInDO::getEmail)
                .one();
