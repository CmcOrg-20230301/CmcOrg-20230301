...
public class SysTenantServiceImpl extends ServiceImpl<SysTenantMapper, SysTenantDO> implements SysTenantService {
    ...
    private void insertOrUpdateSub(SysTenantInsertOrUpdateDTO dto, SysTenantDO sysTenantDO) {
        ...
        if (dto.getId() == null) {
            ...
            addMenuFlag = !sysMenuService.lambdaQuery().eq(BaseEntityNoId::getTenantId, dto.getId())
                .select(BaseEntity::getId).exists();
        ...
        // 新增：字典
        List<SysDictDO> sysDictDOList = sysDictService.lambdaQuery()
            .eq(BaseEntityNoId::getTenantId, BaseConstant.TOP_TENANT_ID).eq(SysDictDO::getSystemFlag, true).list();

        // 执行：新增字典
        doHandleAndAddDict(tenantId, sysDictDOList, true);

        // 新增：参数
        List<SysParamDO> sysParamDOList = sysParamService.lambdaQuery()
            .eq(BaseEntityNoId::getTenantId, BaseConstant.TOP_TENANT_ID).eq(SysParamDO::getSystemFlag, false).list();
    ...
    public Page<SysTenantDO> myPage(SysTenantPageDTO dto) {
        ...
        MyThreadUtil.execute(() -> {
            ...
            // 获取：绑定的菜单
            List<SysMenuDO> sysMenuDOList = sysMenuService.lambdaQuery().in(BaseEntityNoId::getTenantId, idSet)
                .select(BaseEntityNoId::getTenantId).list();

            refMenuCountMap.set(sysMenuDOList.stream()
                .collect(Collectors.groupingBy(BaseEntityNoId::getTenantId, Collectors.counting())));
        ...
        MyThreadUtil.execute(() -> {
            ...
            List<SysUserDO> sysUserDOList = ChainWrappers.lambdaQueryChain(sysUserMapper)
                .in(BaseEntityNoId::getTenantId, idSet).select(BaseEntityNoId::getTenantId).list();

            userCountMap.set(sysUserDOList.stream()
                .collect(Collectors.groupingBy(BaseEntityNoId::getTenantId, Collectors.counting())));
        ...
        MyThreadUtil.execute(() -> {
            ...
            // 获取：字典
            List<SysDictDO> sysDictDOList = sysDictService.lambdaQuery().in(BaseEntityNoId::getTenantId, idSet)
                .select(BaseEntityNoId::getTenantId).list();

            dictCountMap.set(sysDictDOList.stream()
                .collect(Collectors.groupingBy(BaseEntityNoId::getTenantId, Collectors.counting())));
        ...
        MyThreadUtil.execute(() -> {
            ...
            // 获取：参数
            List<SysParamDO> sysParamDOList = sysParamService.lambdaQuery().in(BaseEntityNoId::getTenantId, idSet)
                .select(BaseEntityNoId::getTenantId).list();

            paramCountMap.set(sysParamDOList.stream()
                .collect(Collectors.groupingBy(BaseEntityNoId::getTenantId, Collectors.counting())));
    ...
    public SysTenantInfoByIdVO infoById(NotNullId notNullId) {
        ...
        SysTenantDO sysTenantDO = lambdaQuery().eq(BaseEntity::getId, notNullId.getId())
        ...
        // 获取：绑定的用户 idSet
        List<SysTenantRefUserDO> sysTenantRefUserDOList = sysTenantRefUserService.lambdaQuery()
            .eq(SysTenantRefUserDO::getTenantId, notNullId.getId()).select(SysTenantRefUserDO::getUserId).list();

        Set<Long> userIdSet =
            sysTenantRefUserDOList.stream().map(SysTenantRefUserDO::getUserId).collect(Collectors.toSet());

        // 获取：绑定的菜单 idSet
        List<SysMenuDO> sysMenuDOList = sysMenuService.lambdaQuery().eq(BaseEntityNoId::getTenantId, notNullId.getId())
    ...
    public String addOrderNo(ChangeNumberDTO dto) {
        ...
        List<SysTenantDO> sysTenantDOList = lambdaQuery().in(BaseEntity::getId, dto.getIdSet())
    ...
    public SysSignConfigurationVO getConfigurationById(NotNullLong notNullLong) {
        ...
        MyThreadUtil.execute(() -> {
            ...
            SysUserConfigurationDO sysUserConfigurationDO =
                sysUserConfigurationService.getSysUserConfigurationDoByTenantId(notNullLong.getValue());
        ...
        MyThreadUtil.execute(() -> {
            ...
            GetQrCodeVO getQrCodeVO =
                signWxService.signInGetQrCodeUrl(new UserSignBaseDTO(notNullLong.getValue()), false);
    ...
    public List<SysMenuDO> getSyncMenuInfo(NotNullId notNullId) {
        ...
        // 获取：当前租户的菜单信息
        List<SysMenuDO> sysMenuDOList = allSysMenuDoCollection.stream()
            .filter(it -> it.getTenantId().equals(notNullId.getId())).collect(Collectors.toList());
    ...
    public String doSyncMenu(NotNullIdAndNotEmptyLongSet notNullIdAndNotEmptyLongSet) {
        ...
        // 获取：当前租户的菜单信息
        List<SysMenuDO> sysMenuDOList = sysMenuCacheMap.values().stream()
            .filter(it -> it.getTenantId().equals(tenantId)).collect(Collectors.toList());
    ...
    public String deleteTenantAllMenu(NotEmptyIdSet notEmptyIdSet) {
        ...
        List<SysMenuDO> sysMenuDOList = sysMenuService.lambdaQuery()
            .in(BaseEntityNoIdSuper::getTenantId, notEmptyIdSet.getIdSet()).select(BaseEntity::getId).list();
    ...
    public String doSyncDict() {
        ...
        for (Long tenantId : tenantIdSet) {
            ...
            for (Map.Entry<String, List<SysDictDO>> item : systemDictKeyMap.entrySet()) {
                ...
                // 复制一份新的集合
                List<SysDictDO> newSystemSysDictDOList = item.getValue().stream()
                    .map(it -> BeanUtil.copyProperties(it, SysDictDO.class)).collect(Collectors.toList());
                ...
                List<SysDictDO> tenantAddDictItemSysDictDOList = tenantDictKeySysDictDOList.stream()
                    .filter(it -> !it.getSystemFlag() && it.getType().equals(SysDictTypeEnum.DICT_ITEM)
                        && !systemDictKeySet.contains(it.getUuid()) && !systemDictNameSet.contains(it.getName()))
                    .collect(Collectors.toList());
    ...
    public String doSyncParam() {
        ...
        for (Long tenantId : tenantIdSet) {
            ...
            // 复制一份新的集合
            List<SysParamDO> newDefaultTenantSysParamDOList = defaultTenantSysParamDOList.stream()
                .map(it -> BeanUtil.copyProperties(it, SysParamDO.class)).collect(Collectors.toList());
