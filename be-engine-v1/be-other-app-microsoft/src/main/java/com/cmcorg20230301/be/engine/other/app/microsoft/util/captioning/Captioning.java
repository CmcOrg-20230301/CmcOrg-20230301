...
public class Captioning {
    ...
    private void WriteToConsoleOrFile(String text) throws IOException {
        ...
        if (_userConfig.getOutputFile().isPresent()) {
            try {
                FileWriter outputFile = new FileWriter(_userConfig.getOutputFile().get(), true);
    ...
    public static String GetTimestamp(Instant startTime, Instant endTime, boolean useSubRipTextCaptionFormat) {
        String format = "";
    ...
    public static String StringFromCaption(Caption caption, boolean useSubRipTextCaptionFormat) {
        StringBuilder retval = new StringBuilder();
        if (useSubRipTextCaptionFormat) {
            retval.append(String.format("%d%s", caption.sequence, System.lineSeparator()));
        }
        retval.append(String.format("%s%s", GetTimestamp(caption.begin, caption.end, useSubRipTextCaptionFormat),
    ...
    private String AdjustRealTimeCaptionText(String text, boolean isRecognizedResult) {
        ...
        CaptionHelper captionHelper = new CaptionHelper(Optional.of(_userConfig.getLanguage()),
            _userConfig.getMaxLineLength(), _userConfig.getLines(), new ArrayList<RecognitionResult>());
        ...
        List<String> combinedLines =
            Stream.concat(_recognizedLines.stream(), recognizingLines.stream()).collect(Collectors.toList());
    ...
    private Optional<String> CaptionFromRealTimeResult(SpeechRecognitionResult result, boolean isRecognizedResult) {
        ...
        Instant startTime = CaptionHelper.InstantFromTicks(result.getOffset());
        Instant endTime = CaptionHelper.InstantFromTicks(result.getOffset().add(result.getDuration()));
        ...
        if (_previousEndTime.isPresent() && _previousEndTime.get().compareTo(endTime) > 0) {
            ...
            Caption caption = new Caption(Optional.of(_userConfig.getLanguage()), _srtSequenceNumber++,
                startTime.plusMillis(_userConfig.getDelay()), endTime.plusMillis(_userConfig.getDelay()), "");
            ...
            if (_previousCaption.isPresent()) {
                ...
                retval =
                    Optional.of(StringFromCaption(_previousCaption.get(), _userConfig.getUseSubRipTextCaptionFormat()));
            }
    ...
    private List<Caption> CaptionsFromOfflineResults() {
        ...
        List<RecognitionResult> offlineResults =
            _offlineResults.stream().map(result -> (RecognitionResult)result).collect(Collectors.toList());
        List<Caption> captions = CaptionHelper.GetCaptions(Optional.of(_userConfig.getLanguage()),
            _userConfig.getMaxLineLength(), _userConfig.getLines(), offlineResults);
        ...
        return handleCaptionList(captions, _userConfig.getRemainTime());
    }

    /**
     * 处理：Caption集合
     */
    public static List<Caption> handleCaptionList(List<Caption> captions, int remainTime) {
        ...
        for (int index = 0; index < captions.size() - 1; index++) {
            ...
            Instant end = caption_1.end.plusMillis(remainTime);
        ...
        lastCaption.end = lastCaption.end.plusMillis(remainTime);
    ...
    private void Finish() throws IOException {
        if (CaptioningMode.Offline == _userConfig.getCaptioningMode()) {
            for (Caption caption : CaptionsFromOfflineResults()) {
                WriteToConsoleOrFile(StringFromCaption(caption, _userConfig.getUseSubRipTextCaptionFormat()));
            }
            ...
            if (_previousCaption.isPresent()) {
                ...
                WriteToConsoleOrFile(
                    StringFromCaption(_previousCaption.get(), _userConfig.getUseSubRipTextCaptionFormat()));
            }
        }
    }
    ...
    private void Initialize(UserConfig userConfig) throws IOException {
        ...
        if (_userConfig.getOutputFile().isPresent()) {
            File outputFile = new File(_userConfig.getOutputFile().get());
    ...
    private AudioConfig AudioConfigFromUserConfig() {
        if (_userConfig.getInputFile().isPresent()) {
            if (!_userConfig.getUseCompressedAudio()) {
                WavFileReader reader = new WavFileReader(_userConfig.getInputFile().get());
    ...
    private SpeechConfig SpeechConfigFromUserConfig() {
        final SpeechConfig speechConfig =
            SpeechConfig.fromSubscription(_userConfig.getSubscriptionKey(), _userConfig.getRegion());

        speechConfig.setProfanity(_userConfig.getProfanityOption());

        if (_userConfig.getStablePartialResultThreshold().isPresent()) {
            speechConfig.setProperty(PropertyId.SpeechServiceResponse_StablePartialResultThreshold,
                _userConfig.getStablePartialResultThreshold().get());
        }
    ...
    private void RecognizeContinuous(SpeechRecognizer speechRecognizer)
        ...
        if (CaptioningMode.RealTime == _userConfig.getCaptioningMode()) {
            speechRecognizer.recognizing.addEventListener((s, e) -> {
                try {
                    if (ResultReason.RecognizingSpeech == e.getResult().getReason()
                        && e.getResult().getText().length() > 0) {
                        ...
                        WriteToConsole(
                            String.format("NOMATCH: Speech could not be recognized.%s", System.lineSeparator()));
                    }
        ...
        speechRecognizer.canceled.addEventListener((s, e) -> {
            if (CancellationReason.EndOfStream == e.getReason()) {
                ...
                // Error output should not be suppressed, even if suppress output flag is set.
                System.out.println(String.format("Encountered error.%sError code: %d%sError details: %s%s",
                    System.lineSeparator(), e.getErrorCode(), e.getErrorDetails()));
                done[0] = true; // Notify to stop recognition.
            } else {
                System.out.println(String.format("Request was cancelled for an unrecognized reason: %d.%s",
    ...
    public static void main(String[] args) {
        final String usage = new StringBuilder().append("Usage: java -cp .;target\\dependency\\* Captioning [...]")
            .append(System.lineSeparator()).append(System.lineSeparator()).append("  HELP")
            .append(System.lineSeparator()).append("    --help                           Show this help and stop.")
            .append(System.lineSeparator()).append(System.lineSeparator()).append("  CONNECTION")
            .append(System.lineSeparator())
            .append("    --key KEY                        Your Azure Speech service resource key.")
            .append(System.lineSeparator())
            .append(
                "                                     Overrides the SPEECH_KEY environment variable. You must set the environment variable (recommended) or use the `--key` option.")
            .append(System.lineSeparator())
            .append("    --region REGION                  Your Azure Speech service region.")
            .append(System.lineSeparator())
            .append(
            ...
            .append(System.lineSeparator()).append("                                     Examples: westus, eastus")
            .append(System.lineSeparator()).append(System.lineSeparator()).append("  LANGUAGE")
            .append(System.lineSeparator())
            .append(
                "    --language LANG                  Specify language. This is used when breaking captions into lines.")
            .append(System.lineSeparator()).append("                                     Default value is en-US.")
            .append(System.lineSeparator()).append("                                     Examples: en-US, ja-JP")
            .append(System.lineSeparator()).append(System.lineSeparator()).append("  INPUT")
            .append(System.lineSeparator())
            .append("    --input FILE                     Input audio from file (default input is the microphone.)")
            .append(System.lineSeparator()).append("    --format FORMAT                  Use compressed audio format.")
            .append(System.lineSeparator())
            .append(
                "                                     If this is not present, uncompressed format (wav) is assumed.")
            .append(System.lineSeparator()).append("                                     Valid only with --file.")
            .append(System.lineSeparator())
            .append("                                     Valid values: alaw, any, flac, mp3, mulaw, ogg_opus")
            .append(System.lineSeparator()).append(System.lineSeparator()).append("  MODE")
            .append(System.lineSeparator()).append("    --offline                        Output offline results.")
            .append(System.lineSeparator()).append("                                     Overrides --realTime.")
            .append(System.lineSeparator()).append("    --realTime                       Output real-time results.")
            .append(System.lineSeparator())
            .append("                                     Default output mode is offline.")
            .append(System.lineSeparator()).append(System.lineSeparator()).append("  ACCURACY")
            .append(System.lineSeparator())
            .append("    --phrases \"PHRASE1;PHRASE2\"    Example: \"Constoso;Jessie;Rehaan\"")
            .append(System.lineSeparator()).append(System.lineSeparator()).append("  OUTPUT")
            .append(System.lineSeparator()).append("    --output FILE                    Output captions to text file.")
            .append(System.lineSeparator())
            .append(
            ...
            .append(System.lineSeparator())
            .append("                                     Minimum is 20. Default is 37 (30 for Chinese).")
            .append(System.lineSeparator())
            .append("    --lines LINES                    Set the number of lines for a caption to LINES.")
            .append(System.lineSeparator()).append("                                     Minimum is 1. Default is 2.")
            .append(System.lineSeparator())
            .append(
                "    --delay MILLISECONDS             How many MILLISECONDS to delay the appearance of each caption.")
            .append(System.lineSeparator())
            .append("                                     Minimum is 0. Default is 1000.")
            .append(System.lineSeparator())
            .append(
            ...
            .append("                                     Minimum is 0. Default is 1000.")
            .append(System.lineSeparator())
            .append("    --quiet                          Suppress console output, except errors.")
            .append(System.lineSeparator())
            .append("    --profanity OPTION               Valid values: raw, remove, mask")
            .append(System.lineSeparator()).append("                                     Default value: mask")
            .append(System.lineSeparator())
            .append("    --threshold NUMBER               Set stable partial result threshold.")
            .append(System.lineSeparator()).append("                                     Default value: 3")
        ...
        try {
            ...
            if (argsList.contains("--help")) {
                ...
                final Captioning captioning = new Captioning();
