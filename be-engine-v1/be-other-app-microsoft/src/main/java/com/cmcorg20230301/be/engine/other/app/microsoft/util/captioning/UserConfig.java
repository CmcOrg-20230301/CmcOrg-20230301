...
public final class UserConfig {
    ...
    public UserConfig(boolean useCompressedAudio, AudioStreamContainerFormat compressedAudioFormat,
        ProfanityOption profanityOption, String language, Optional<String> inputFile, Optional<String> outputFile,
        Optional<String> phraseList, boolean suppressConsoleOutput, CaptioningMode captioningMode, int remainTime,
        int delay, boolean useSubRipTextCaptionFormat, int maxLineLength, int lines,
        Optional<String> stablePartialResultThreshold, String subscriptionKey, String region) {
    ...
    static private Optional<String> GetCmdOption(List<String> args, String option) {
        final int index =
            args.stream().map(arg -> arg.toLowerCase()).collect(Collectors.toList()).indexOf(option.toLowerCase());
        if (index > -1 && index < args.size() - 1) {
            // We found the option (for example, "--output"), so advance from that to the value (for example,
            // "filename").
    ...
    static private boolean CmdOptionExists(List<String> args, String option) {
        return (args.stream().map(arg -> arg.toLowerCase()).collect(Collectors.toList())
    ...
    static public UserConfig UserConfigFromArgs(List<String> args, String usage) throws IllegalArgumentException {
        ...
        if (0 == key.length()) {
            throw new IllegalArgumentException(String.format(
                "Please set the SPEECH_KEY environment variable or provide a Speech resource key with the --key option.%s%s",
        ...
        if (0 == region.length()) {
            throw new IllegalArgumentException(String.format(
                "Please set the SPEECH_REGION environment variable or provide a Speech resource region with the --region option%s%s",
                System.lineSeparator(), usage));
        }
        ...
        CaptioningMode captioningMode = CmdOptionExists(args, "--realTime") && !CmdOptionExists(args, "--offline")
            ? CaptioningMode.RealTime : CaptioningMode.Offline;
        ...
        return new UserConfig(CmdOptionExists(args, "--format"), GetCompressedAudioFormat(args),
            GetProfanityOption(args), language, GetCmdOption(args, "--input"), GetCmdOption(args, "--output"),
            GetCmdOption(args, "--phrases"), CmdOptionExists(args, "--quiet"), captioningMode, remainTime, delay,
            CmdOptionExists(args, "--srt"), maxLineLength, lines, GetCmdOption(args, "--threshold"), key, region);
    }
}
