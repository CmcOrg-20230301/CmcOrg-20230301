...
public class SysSocketRefUserServiceImpl extends ServiceImpl<SysSocketRefUserMapper, SysSocketRefUserDO>
    ...
    public String offlineByIdSet(NotEmptyIdSet notEmptyIdSet) {
        ...
        // 检查：是否非法操作
        SysTenantUtil.checkIllegal(notEmptyIdSet.getIdSet(), tenantIdSet -> lambdaQuery()
            .in(BaseEntity::getId, notEmptyIdSet.getIdSet()).in(BaseEntityNoId::getTenantId, tenantIdSet).count());
        ...
        if (CollUtil.isNotEmpty(sysSocketRefUserDOList)) {
            ...
            for (SysSocketRefUserDO sysSocketRefUserDO : sysSocketRefUserDOList) {
                ...
                CacheRedisKafkaLocalUtil.put(sysSocketRefUserDO.getJwtHash(), sysSocketRefUserDO.getJwtHashExpireTs(),
                    () -> "不可用的 jwt：下线");
    ...
    public String changeConsoleFlagByIdSet(NotEmptyIdSet notEmptyIdSet) {
        ...
        List<SysSocketRefUserDO> sysSocketRefUserDOList =
            lambdaQuery().in(BaseEntity::getId, notEmptyIdSet.getIdSet()).select(SysSocketRefUserDO::getUserId).list();

        Set<Long> userIdSet =
            sysSocketRefUserDOList.stream().map(SysSocketRefUserDO::getUserId).collect(Collectors.toSet());
