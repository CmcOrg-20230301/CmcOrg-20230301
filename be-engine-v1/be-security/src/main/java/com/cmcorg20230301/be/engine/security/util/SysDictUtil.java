...
public class SysDictUtil {
    ...
    public static List<DictIntegerVO> listByDictKey(String dictKey) {
        ...
        Map<Long, Map<String, List<DictIntegerVO>>> dictMap =
            MyCacheUtil.getMap(BaseRedisKeyEnum.SYS_DICT_CACHE,
                CacheHelper.getDefaultLongMapStringListMap(), () -> {

                    return ChainWrappers.lambdaQueryChain(sysDictMapper)
                        ...
                        .stream()
                        .collect(Collectors.groupingBy(BaseEntityNoId::getTenantId, Collectors
                            .groupingBy(SysDictDO::getDictKey, Collectors
                                .mapping(it -> new DictIntegerVO(it.getValue(), it.getName()),
                                    Collectors.toList()))));
